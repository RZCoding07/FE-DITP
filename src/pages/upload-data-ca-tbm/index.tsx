  ...
  useEffect(() => {
    if (values.length > 0) {
        const mapped = values.map((value) => ({
            ...
            ca: { label: value[4], value: slugify(value[4], { lower: true }) },
            measurement: value[5]
        }));
  ...
  return (
    <Layout>
      ...
      <Layout.Body>
        <Card>
          <CardHeader>
            <CardTitle>Upload Data Corrective Action</CardTitle>
            <div className='flex items-center justify-between'>
              <p className='text-muted-foreground'>
                Upload file .csv atau .xlsx untuk menambahkan data Corrective Action
              </p>
            </div>
            <div className='flex items-center justify-between'>
              <button
