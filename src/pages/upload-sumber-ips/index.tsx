...
export default function UploadUser() {
  ...
  const handleUploadUsers = async () => {
    ...
    const apiUrl = import.meta.env.VITE_API_MONICA as string
    ...
    for (let i = 0; i < 10; i++) {
      ...
      uploadPromises.push(
        fetch(`${apiUrl}/monica/uploadSumberIps`, {
          method: 'POST',
          headers: {
  ...
  return (
    <Layout>
      ...
      <Layout.Body>
        <Card>
          <CardHeader>
            <CardTitle>Upload IPS</CardTitle>
            <div className='flex items-center justify-between'>
              <p className='text-muted-foreground'>
                Upload file .csv atau .xlsx untuk menambahkan data IPS
              </p>
            </div>
            ...
            <div className='mt-5 grid'>
              ...
              <div className='mt-5 flex items-center justify-end gap-3'>
                ...
                <button className='cursor-pointer rounded-full bg-slate-600 px-5 py-[.675rem] text-center text-sm font-semibold text-white transition duration-300 ease-in-out hover:bg-slate-800 focus:outline-none focus:ring-4 focus:ring-slate-300 disabled:cursor-not-allowed'>
                  <Link to='/sumber-ips'>Kembali</Link>
                </button>
              </div>
            </div>
          </CardContent>
