...
import { fetchDistinctYearsSerapanBiaya, fetchKebun, fetchSerapanBiaya } from '@/utils/api_immature'
...
export default function PicaTbm() {
    ...
    const [rekapTotalSeleksiPerKebun, setRekapTotalSeleksiPerKebun] = useState<any[]>([])
    ...
    useEffect(() => {
        const fetchProcVegetatifDefault = async () => {
            ...
            try {
                ...
                setScoresKebun([]);  // reset scores array
                setRekapTotalSeleksiPerKebun([])
                setSerapanBiaya([])  // reset serapanBiaya array
                setColorData({
                    hitam: 0,
                    merah: 0,
                    hijau: 0,
                ...
                let bulanVal = 0
                let tahunVal = 0
                if(parseInt(bulan.value) === 1){
                    bulanVal = 12
                    tahunVal = parseInt(tahun.value) - 1
                } else if(parseInt(bulan.value) > 4){
                    bulanVal = 4
                    tahunVal = parseInt(tahun.value)
                } else if(parseInt(bulan.value) > 8){
                    bulanVal = 8
                    tahunVal = parseInt(tahun.value)
                } else {
                    bulanVal = 12
                ...
                for (let i = 1; i < 4; i++) {
                        ...
                        setScoresKebun((prev) => [
                            ...prev,
                            {
                                [`tbm${i}`]: {
                                    ...
                                    kebun,
                                    luas : parseFloat(newScoresPerKebun[kebun].totalLuas.toFixed(2)),
                                    totalSeleksiKebun: parseFloat(newScoresPerKebun[kebun].totalSeleksiKebun.toFixed(2)),
                    ...
                    setRekapTotalSeleksiPerKebun((prev) => [
                        ...prev,
                        {
                            [`tbm${i}`]: newScoresPerKebun,
                        },
                    ]);


                    console.log('newScoresPerKebun', newScoresPerKebun)
    ...
    interface TotalSeleksiPerKebun {
        [key: string]: KebunData
    }


    const [processedData, setProcessedData] = useState<TotalSeleksiPerKebun>({})

    useEffect(() => {
        if (rekapTotalSeleksiPerKebun.length > 0) {
            const totalSeleksiPerKebun: TotalSeleksiPerKebun = {};
    
            // First pass: Collect total area and count TBMs for each kebun
            rekapTotalSeleksiPerKebun.forEach((item: any) => {
                Object.values(item).forEach((tbmData: any) => {
                    Object.values(tbmData).forEach((kebun: any) => {
                        const { kebun: kebunName, regional, totalLuas } = kebun;
    
                        if (!totalSeleksiPerKebun[kebunName]) {
                            totalSeleksiPerKebun[kebunName] = {
                                regional,
                                kebun: kebunName,
                                totalSeleksiKebun: 0,
                                totalLuas: 0,
                        ...
                        totalSeleksiPerKebun[kebunName].totalLuas += totalLuas;
                        totalSeleksiPerKebun[kebunName].tbmCount += 1;
                    });
                });
            });
    
            // Second pass: Calculate proportional totalSeleksiKebun
            rekapTotalSeleksiPerKebun.forEach((item: any) => {
                Object.values(item).forEach((tbmData: any) => {
                    Object.values(tbmData).forEach((kebun: any) => {
                        const { kebun: kebunName, totalSeleksiKebun, totalLuas } = kebun;
                        
                        const proportion = totalLuas / totalSeleksiPerKebun[kebunName].totalLuas;
                        totalSeleksiPerKebun[kebunName].totalSeleksiKebun += totalSeleksiKebun * proportion;
                    });
                });
            });

            
            setProcessedData(totalSeleksiPerKebun)
            

        }
    }, [rekapTotalSeleksiPerKebun]);


    useEffect(() => {
        const fetchBulanTahun = async () => {
            try {
                const data = await fetchDistinctYearsSerapanBiaya()
